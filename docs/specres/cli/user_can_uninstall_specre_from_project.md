...
- `remove_markers_from_file(path: &Path) -> Result<usize, SpecreError>` — strips all lines containing `@specre <ULID>` from a single file, returning the count of lines removed; rewrites the file only when at least one line was removed
...
5. The command scans all source files in all configured `source_dirs` and removes every line that contains an `@specre <ULID>` marker, rewriting each file only when at least one marker line was removed.
...
A source file that contains no `@specre` line is read but not rewritten, and is not mentioned in the output summary.

### Scenario 5: Source file with multiple markers — all removed

A source file containing two or more `@specre` lines has every such line removed in a single rewrite. The removed count reflects the total lines stripped from that file.
