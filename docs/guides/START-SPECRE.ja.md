...
specre エコシステムが健全で安定したら、`specre health-check` を CI パイプラインに組み込み、サイレントな劣化を防ぎましょう。**[CI インテグレーションガイド](CI-INTEGRATION.ja.md)** で、GitHub Actions ワークフロー、閾値チューニング、その他の CI プラットフォームについて解説しています。
