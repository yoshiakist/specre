...
fn call_orphans(stdin: &mut impl std::io::Write, reader: &mut BufReader<impl std::io::Read>, id: u64) -> Value {
    send(stdin, &json!({
        "jsonrpc": "2.0",
        "id": id,
        "method": "tools/call",
...
fn mcp_tool_orphans_clean() {
    ...
    create_specre_card(&dir, "docs/specres", "cli/card_a.md", "01AAAAAAAAAAAAAAAAAAAAAAAA", "card_a", "deprecated");
...
fn mcp_tool_orphans_with_dangling() {
    ...
    dir.child("src/main.rs").write_str("// @specre 01ZZZZZZZZZZZZZZZZZZZZZZZZ\nfn main() {}\n").unwrap();
